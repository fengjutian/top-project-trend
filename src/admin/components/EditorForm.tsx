...
import {Sparkles, Wand2, Image as ImageIcon, BookOpen, Clock, ListOrdered, Save, ChevronDown} from 'lucide-react';
...
export default function EditorForm({article, onUpdate, onApplyTemplate, metrics, onPickMediaForImage}: EditorFormProps) {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-7 pb-24 pt-8 reading-prose">
      ...
      {/* Meta panel — collapsed by default */}
      <details className="group rounded-lg border border-border bg-card">
        <summary className="flex cursor-pointer list-none items-center justify-between px-4 py-3 [&::-webkit-details-marker]:hidden">
          <span className="font-serif text-sm font-semibold text-foreground">文章设置</span>
          <span className="flex items-center gap-3 text-xs text-muted-foreground">
            <span>{article.date || '日期'} · {article.slug || '未生成链接'} · {article.draft ? '草稿' : '已发布'}</span>
